...
코로케이션 게이트웨이로부터 유입된 모든 트래픽을 처리할 VM 인스턴스의 Port 및 가상 IP 포트를 지정할 수 있습니다. 라우트의 게이트웨이가 되는 VM 인스턴스에는 Network Virtual Appliance VM를 배치하여 VM 인스턴스 내부에서 트래픽을 제어하고 피어링 기능을 이용하여 다른 VPC로 트래픽을 전달할 수 있습니다.<br>
* 피어링으로 허브 & 스포크(Hub & Spoke) 형태의 VPC 연결을 구성하고 허브 VPC에 위치한 Network Virtual Appliance로 모든 트래픽을 제어하는 상황에서 고객의 온프레미스 네트워크를 허브 VPC에 스포크로 추가하기 위해 코로케이션 게이트웨이의 라우트 기능을 활용할 수 있습니다. 

> [참고] 현재는 한국(평촌), 한국(판교) 리전에서만 기능이 제공됩니다.

#### 라우트 생성

1. 라우트 설정을 원하는 코로케이션 게이트웨이 선택
2. 하단 탭에서 라우트 선택
3. **라우트 생성** 선택
4. 게이트웨이 선택 후 확인 버튼을 클릭합니다
   > [참고] 게이트웨이는 인스턴스와 가상 IP만 선택이 가능합니다.<br>

#### 라우트 삭제

1. 라우트 설정 삭제를 원하는 코로케이션 게이트웨이 선택
2. 라우트 선택
3. **라우트 삭제** 버튼을 클릭 합니다.
